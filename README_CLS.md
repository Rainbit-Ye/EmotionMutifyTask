...
| 多任务版 | 慢 | 标准+ | 困难样本、混淆情绪 |
  ...
  这是一个基于 RoBERTa 的情绪分类项目，支持 7 种情绪分类。项目包含 三种训练方法：
  ...
  │ 方法         │ 文件                     │ 特点                                  │
  ├──────────────┼──────────────────────────┼───────────────────────────────────────┤
  │ 基础分类训练 │ cls_trainer.py           │ 使用 LoRA 微调，类别权重处理不平衡    │
  ├──────────────┼──────────────────────────┼───────────────────────────────────────┤
  │ 多任务学习   │ cls_multitask_trainer.py │ 主任务+辅助任务，Focal Loss，对比学习 │
  ├──────────────┼──────────────────────────┼───────────────────────────────────────┤
  │ 简单基线     │ simple_trainer.py        │ 全参数微调，作为对比基线              │
  └──────────────┴──────────────────────────┴───────────────────────────────────────┘
  ...
  - 加载原始对话数据
  - 转换为 SFT 格式
  - 数据平衡处理（neutral 欠采样，其他过采样）
  - 计算类别权重

  2. 模型训练
  python cls_trainer.py           # 基础训练
  python cls_multitask_trainer.py # 多任务训练
  python simple_trainer.py        # 简单基线
  ...
  4. 每个 epoch 的流程
  1. 数据加载 → DataLoader
  2. 前向传播 → 计算 loss（带类别权重）
  3. 反向传播 → 梯度裁剪 → 优化器更新
  4. 验证评估 → 计算准确率
  5. 保存模型 → checkpoint / best / final

  5. 模型评估
  python cls_evaluate.py --eval_cls --eval_simple

  ---
  三、关键技术

  - LoRA 微调：只训练 query, value, key, dense 层
  - 类别权重：处理数据不平衡（neutral 权重 2.0）
  - 学习率调度：warmup 10% + 线性衰减
  - 多任务损失：L = L_main + α*L_turn + β*L_consistency + γ*L_contrastive
